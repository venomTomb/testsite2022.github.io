...
getSellRequests()
mainContract.methods.soldStakeFunds(user.address).call({shouldPollResponse:true}).then(res=>{$('.tmn-12')[0].innerHTML=`You have - ${parseInt(res)/1e18} MATIC - collectable from your sold stakes! click to collect.`})}
...
mainContract.methods.sellStakeRequest(currentTargetStakeId,val).send({from:user.address,shouldPollResponse:true}).then(res=>{console.log(res)
doAlert(`Successfully created sell offer for ${parseFloat($('.sell-price')[0].value)} MATIC`,3)
closeModal()
...
$('.grid-container-fs')[0].innerHTML+=`
        ...
        ">${calcBonusToken(parseInt(itm.endDay)-parseInt(itm.startDay),parseInt(itm.tokenValue)/1e18).toFixed(2)} RIP</span><div class="v-i-77-${ide} v-i-7" onclick="buyAStake('${itm.userAddress}', ${itm.stakeId})" style="
            ...
            /* display: none; */
            margin-top: 66px;
        ">Buy For ${(parseInt(itm.price)/1e18).toFixed(4)} MATIC</div></div></div>
        `
if((itm.userAddress).toLocaleUpperCase()===(user.address).toLocaleUpperCase()){$(`.v-i-77-${ide}`)[0].innerHTML=`Selling For ${(parseInt(itm.price)/1e18).toFixed(4)} MATIC`
$(`.v-i-77-${ide}`)[0].style.background="linear-gradient(to bottom right, #cddc39 0%, #4caf50 100%)"}
getStakeDivs(itm.stakeId,ide)
function getStakeDivs(id,ide){mainContract.methods.calcStakeCollecting(itm.userAddress,id).call({shouldPollResponse:true}).then(res=>{$(`.v-i-5-${ide}`)[0].innerHTML=(parseInt(res)/1e18).toFixed(3)+" MATIC"})}}})}
...
mainContract.methods.mapMemberStake(sellerAddress,stakeId).call({shouldPollResponse:true}).then(res=>{$('.mod-txt')[0].innerHTML="You are buying A stake for "+(parseInt(res.price)/1e18).toFixed(5)+" MATIC"
